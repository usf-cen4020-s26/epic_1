...
01  WS-TEMP-EXP-TITLE           PIC X(50).
01  WS-TEMP-EXP-COMPANY         PIC X(50).
01  WS-TEMP-EXP-DATES           PIC X(30).
01  WS-TEMP-EXP-DESC            PIC X(100).
...
*> *      *>*****************************************************************
       7311-GET-SINGLE-EXPERIENCE.
           ...
           IF WS-EOF-FLAG = 1
               EXIT PARAGRAPH
           END-IF.
           MOVE INPUT-RECORD TO WS-TEMP-EXP-COMPANY.
           MOVE WS-TEMP-EXP-COMPANY TO WS-OUTPUT-LINE.
           PERFORM 8000-WRITE-OUTPUT.
           ...
           IF WS-EOF-FLAG = 1
               EXIT PARAGRAPH
           END-IF.
           MOVE INPUT-RECORD TO WS-TEMP-EXP-DATES.
           MOVE WS-TEMP-EXP-DATES TO WS-OUTPUT-LINE.
           PERFORM 8000-WRITE-OUTPUT.
           ...
           IF WS-EOF-FLAG = 1
               EXIT PARAGRAPH
           END-IF.
           MOVE INPUT-RECORD TO WS-TEMP-EXP-DESC.
           IF WS-TEMP-EXP-DESC NOT = SPACES
               MOVE WS-TEMP-EXP-DESC TO WS-OUTPUT-LINE
               PERFORM 8000-WRITE-OUTPUT
           ELSE
               MOVE "(skipped)" TO WS-OUTPUT-LINE
               PERFORM 8000-WRITE-OUTPUT
...
*> *      *>*****************************************************************
       7321-GET-SINGLE-EDUCATION.
           ...
           IF WS-EOF-FLAG = 1
               EXIT PARAGRAPH
           END-IF.
           MOVE INPUT-RECORD TO WS-TEMP-EDU-UNIVERSITY.
           MOVE WS-TEMP-EDU-UNIVERSITY TO WS-OUTPUT-LINE.
           PERFORM 8000-WRITE-OUTPUT.
           ...
           IF WS-EOF-FLAG = 1
               EXIT PARAGRAPH
           END-IF.
           MOVE INPUT-RECORD TO WS-TEMP-EDU-YEARS.
           MOVE WS-TEMP-EDU-YEARS TO WS-OUTPUT-LINE.
           PERFORM 8000-WRITE-OUTPUT.
...
*> *      *>*****************************************************************
       7410-SAVE-EXPERIENCE-ENTRIES.
           IF WS-EXP-LOOP-INDEX >= 1
               MOVE WS-TEMP-EXP-TITLE TO
                   WS-EXP-TITLE(WS-CURRENT-PROFILE-INDEX, 1)
               MOVE WS-TEMP-EXP-COMPANY TO
                   WS-EXP-COMPANY(WS-CURRENT-PROFILE-INDEX, 1)
               MOVE WS-TEMP-EXP-DATES TO
                   WS-EXP-DATES(WS-CURRENT-PROFILE-INDEX, 1)
               MOVE WS-TEMP-EXP-DESC TO
                   WS-EXP-DESC(WS-CURRENT-PROFILE-INDEX, 1)
           END-IF.
...
*> *      *>*****************************************************************
       7420-SAVE-EDUCATION-ENTRIES.
           IF WS-EDU-LOOP-INDEX >= 1
               MOVE WS-TEMP-EDU-DEGREE TO
                   WS-EDU-DEGREE(WS-CURRENT-PROFILE-INDEX, 1)
               MOVE WS-TEMP-EDU-UNIVERSITY TO
                   WS-EDU-UNIVERSITY(WS-CURRENT-PROFILE-INDEX, 1)
               MOVE WS-TEMP-EDU-YEARS TO
                   WS-EDU-YEARS(WS-CURRENT-PROFILE-INDEX, 1)
           END-IF.
